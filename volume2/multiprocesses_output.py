...
def multiprocess(input_tuple):
    list_color = ""
    index,BVH,image_height, image_width, samples_per_pixel,max_depth ,camera_center, defocus_angle, defocus_disk_v,defocus_disk_u, pixel_u, pixel_v, ray_tmin, ray_tmax, pixel00_loc,saturation, background , processes = input_tuple
    out_string = ""
    ...
    for j in range((image_height * index) // processes, (image_height * (index + 1)) // processes):
        for i in range(image_width):
            ...
            for n in range(samples_per_pixel):
                ...
                pixel_color, depth = ray_color(r,ray_tmin, ray_tmax, max_depth, BVH, background)
                current_color += pixel_color
                total_depth_reached += depth
                count += 1
            
            current_color /= samples_per_pixel
            current_color = current_color**saturation
            list_color+=(append_color(current_color))
    return (index, list_color, total_depth_reached/count )