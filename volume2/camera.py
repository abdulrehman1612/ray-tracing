...
class camera:
    def __init__(self, aspect_ratio, image_width, lookfrom:point3 = point3(0,0,0), lookat:point3 = point3(0,0,-1), zoom: float = 1.0 , rotate_camera: int = 0,defocus_angle = 0, focus_distance = 10 ,ray_tmin = 0.001, ray_tmax = float('inf'), samples_per_pixel:int=5, saturation:float = 1, background_color = color(0,0,0), max_depth:int = 10):
             ...
             for j in range(self.image_height):
                 for i in range(self.image_width):
                    ...
                    for n in range(self.samples_per_pixel):
                        ...
                        pixel_color = ray_color(r,self.ray_tmin, self.ray_tmax, self.max_depth, BVH, self.background_color)
    ...
    def render_multicore(self,world,out_file = "image.ppm", processes=12, tasks = 100):
        ...
        for task in range(tasks):
            list_input.append((task,BVH, self.image_height, self.image_width, self.samples_per_pixel, self.max_depth,self.camera_center, self.defocus_angle, self.defocus_disk_v,self.defocus_disk_u, self.pixel_u, self.pixel_v, self.ray_tmin, self.ray_tmax, self.pixel00_loc,self.saturation,self.background_color , tasks))
        ...
        with Pool(processes) as p:
            for output in p.imap_unordered(multiprocess, list_input):
                ...
                minutes = (elapsed % 3600) // 60
                seconds = elapsed % 60
                total_avg_depth += output[2]
                count += 1
                
                print(f"Elapsed_time: {hours:02d}:{minutes:02d}:{seconds:02d} | Progress: {round((tasks_done/tasks)*100,2)} % | Avg_depth: {output[2]}")
        ...
        print("Progress: Complete")
        print(f"Time taken: {hours:02d}:{minutes:02d}:{seconds:02d}")
        print(f"Total Average depth: {total_avg_depth/count}")
